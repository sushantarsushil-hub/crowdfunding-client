...
export default function CreatorPaymentHistoryPage() {
  ...
  const { data: response, isLoading, isError, refetch } = useQuery({
    queryKey: ['creatorPaymentHistory', page],
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Payment & Transaction History</h1>
        <p className="text-xs text-slate-500 font-medium mt-0.5">
          Record of all your credit purchases and transactions.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <ErrorState
          ...
          message="Could not connect to transaction ledger."
          onRetry={refetch}
        />
      ) : payments.length === 0 ? (
        <EmptyState
          title="No payment records found"
          description="You have no recorded payment transactions."
        />
      ) : (
